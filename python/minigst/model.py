...
def prepare_likelihood(db, vname, model, pol_drift=None, ext_drift=None, type = "ordinary"):
...
def model_mle(
    ...
    while keepgoing:
      ...
      ll = gl.AModelOptimFactory.create(model,dbaux,None,None,None,mop = mop, nb_neighVecchia = nVecchia, reml = reml)
      cost = ll.run()
      if prune_proportion <= 0:
        break
      keepgoing = pruneModelF(model,prop_var_min = prune_proportion)

    return {'model' : model, 'driftCoeffs' : model.getDriftList().getBetaHats(), 'likelihood' : -cost}
