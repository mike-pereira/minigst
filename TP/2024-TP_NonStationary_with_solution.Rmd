...
minikriging(data, target, vname = "log_cadmium", extDrift ='dist.m', model = model_drift, type = "universal", prefix = "UK", neighborhood = "unique", std = TRUE)
...
err = xvalidation(data, vname = "log_cadmium", drift ='dist.m', model=model_drift, type = "universal",prefix ="UK", neighborhood = "unique", std = TRUE)
...
dbplot_grid(target,color = "UK*estim")
dbplot_point(data,size = "*UK.log_cadmium.esterr", add=T)
...
require(mltools)
require(data.table)
newdata <- one_hot(as.data.table(meuse[,c("soil","ffreq")]))
meuse <- cbind(meuse,as.data.frame(newdata)[,-c(3,6)])

newdata <- one_hot(as.data.table(meuse.grid[,c("soil","ffreq")]))
meuse.grid <- cbind(meuse.grid,as.data.frame(newdata)[,-c(3,6)])
