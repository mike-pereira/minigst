...
2.  Plot each heavy metal (log)concentration. *(Indication: Use the function `db_plot_point`)*
...
3.  Compute basic statistics of each heavy metal (log)concentration. *(Indication: Use the function `summaryStats`)*
...
4.  Compare the basic statistics above to those obtained when considering only the points having a Cadmium log-concentration above 0.5. *(Indication: Use the functions `addSel`, and `summaryStats`)*
...
4.  Fit the regression line of the coordinates **y** as a function of the coordinates **x**. Compare the basic statistics of each heavy metal concentration above and below the regression line. *(Indication: Use the functions `lm`, `addSel`, and `summaryStats`)*
...
1.  Create a matrix `dist` containing all the distances between the points and a matrix `vario` containing the quantities $$\frac{1}{2}(Z(x_i)-Z(x_j))^2$$ for all the pairs of points $i$ and $j$ (using the function `outer`). You may choose to work with the cadmium concentration for this example. *(Indication: Use the function `outer`)*
